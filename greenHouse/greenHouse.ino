...
#define ssid "Mi6"             //wifi network name
#define pwd  "chenyf0328"     //wifi password
...
//  // test K records
//  for (int i=1; i<=K; i++){
//    Serial.println(String(i, DEC) + ". " + records[i]);
//  }
//  Serial.println();
}
...
static void outputHTML ( WiFiClient& client ) {
    ...
    client.println( "  <body>" );
    client.println( "    <h2> 25 records of light and temp </h2>" );

    // use loop to print K records
    for (int i=1; i<=K; i++){
      client.println( "    <p>" + String(i, DEC) + ". " + records[i] + "</p>" );
    }
